...
function uploadfile()
{
  ...
  if( file !== '')
  {
    ...
    $.ajax({
      url:BASE_URL + 'masters/products/import_items',
      type:"POST",
      cache:"false",
      data: fd,
      processData:false,
      ...
      success: function(rs){
        ...
        if(isJson(rs)) {
          ...
          let summary = `นำเข้าทั้งหมด ${ds.count} <br/>เพิ่มใหม่ ${ds.add}<br/>อัพเดต ${ds.update}<br/>ไม่เข้า ${ds.failed}<br/>`;
